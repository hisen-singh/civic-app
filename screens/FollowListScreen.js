  ...
  const loadUsers = async (isRefresh = false) => {
    ...
    try {
      const result = listType === 'followers'
        ? await UserService.getFollowers(userId, 20, isRefresh ? null : lastDoc)
        : await UserService.getFollowing(userId, 20, isRefresh ? null : lastDoc);

      setUsers(prev => isRefresh ? result.users : [...prev, ...result.users]);
  ...
  const renderUser = ({ item }) => (
    ...
    >
      ...
      <View style={styles.userInfo}>
        <Text style={styles.displayName}>{item.displayName || 'Civic User'}</Text>
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <Text style={styles.header}>
            {listType === 'followers' ? `${users.length} Followers` : `Following ${users.length}`}
          </Text>
        }
        ListEmptyComponent={
          !loading && (
