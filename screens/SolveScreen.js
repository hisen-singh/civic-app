...
export default function SolveScreen() {
    ...
    const fetchIssues = async (isRefresh = false) => {
        try {
            ...
            const mySolves = allIssues.filter(i =>
                (i.solvers || []).includes(uid) && i.status === 'Solved'
            );
            const myActive = allIssues.filter(i =>
                (i.solvers || []).includes(uid) && i.status === 'In Progress'
            );
    ...
    const getDifficulty = (urgency) => DIFFICULTY_MAP[urgency] || DIFFICULTY_MAP.medium;

    const urgentTaskCount = issues.filter(i => i.urgency === 'critical' || i.urgency === 'high').length;
    ...
    const renderStatsHeader = () => (
        <Animated.View style={[styles.statsSection, { 
            ...
            <View style={styles.statsRow}>
                <LinearGradient colors={['rgba(99,102,241,0.15)', Colors.surface]} style={[styles.statCard, styles.statCardAccent]}>
                    <View style={styles.statIconWrap}>
                        <MaterialCommunityIcons name="trophy-outline" size={20} color={Colors.accentLight} />
                    </View>
                    <Text style={styles.statValue}>{solvedCount}</Text>
                    <Text style={styles.statLabel}>Solved</Text>
                </LinearGradient>
                <View style={styles.statCard}>
                    <View style={[styles.statIconWrap, { backgroundColor: Colors.warningSurface }]}>
            ...
            <View style={styles.taskCountRow}>
                <Text style={styles.taskCountText}>
                    {sortedIssues.length} {sortedIssues.length === 1 ? 'task' : 'tasks'} available
                </Text>
                <View style={styles.xpPreviewChip}>
                    ...
                    <Text style={styles.xpPreviewText}>
                        Up to {sortedIssues.reduce((sum, i) => sum + getDifficulty(i.urgency).xp, 0)} XP
                    </Text>
                </View>
            </View>
        </Animated.View>
                ...
                <View style={styles.rewardBanner}>
                    <View style={styles.rewardLeft}>
                        <View style={[styles.difficultyBadge, { backgroundColor: difficulty.color + '20' }]}>
                            <MaterialCommunityIcons name={difficulty.icon} size={12} color={difficulty.color} />
