...
import { View, ScrollView, TouchableOpacity, Image, Alert, Animated, KeyboardAvoidingView, Platform } from 'react-native';
...
export default function EditProfileScreen({ navigation }) {
    ...
    const hasChanges = displayName.trim() !== (user?.displayName || '') || localPhoto !== null;
    ...
    return (
        <Animated.View style={{ flex: 1, backgroundColor: Colors.background, opacity: fadeAnim }}>
            <View style={styles.header}>
                ...
                <TouchableOpacity onPress={handleSave} disabled={saving || !hasChanges} activeOpacity={0.7} style={{ padding: 8 }}>
                    <Text style={[styles.saveBtn, (!hasChanges || saving) && { opacity: 0.35 }]}>
                        {saving ? 'Saving...' : 'Save'}
                    </Text>
                </TouchableOpacity>
            </View>
            ...
            {successMsg ? (
                <Animated.View style={[styles.successToast, { opacity: successAnim, transform: [{ translateY: successAnim.interpolate({ inputRange: [0, 1], outputRange: [-20, 0] }) }] }]}>
                    ...
                    <Text style={{ color: Colors.success, fontSize: 13, fontWeight: '600' }}>{successMsg}</Text>
                </Animated.View>
            ) : null}
                ...
                <ScrollView contentContainerStyle={{ padding: Spacing.xl, paddingBottom: 60 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                    ...
                    <View style={styles.fieldSection}>
                        ...
                        <TextInput
                            ...
                            mode="outlined" style={styles.input} textColor={Colors.textPrimary}
                    ...
                    <View style={styles.fieldSection}>
                        ...
                        <View style={styles.readOnlyField}>
                            <MaterialCommunityIcons name="email-outline" size={18} color={Colors.textTertiary} style={{ marginRight: 12 }} />
                            <Text style={styles.readOnlyText} numberOfLines={1}>{user?.email || ''}</Text>
                            <View style={[styles.verifiedBadge, isVerified ? styles.verifiedActive : styles.verifiedInactive]}>
                                <MaterialCommunityIcons name={isVerified ? "check-decagram" : "alert-circle-outline"} size={12} color={isVerified ? Colors.success : Colors.warning} style={{ marginRight: 4 }} />
                                <Text style={{ fontSize: 10, fontWeight: '700', color: isVerified ? Colors.success : Colors.warning }}>
                                    {isVerified ? 'VERIFIED' : 'UNVERIFIED'}
                                </Text>
                            </View>
                        </View>
                        {!isVerified && (
                            <TouchableOpacity onPress={handleResendVerification} disabled={resendingEmail} activeOpacity={0.7} style={styles.resendBtn}>
                                {resendingEmail ? <ActivityIndicator size={14} color={Colors.accent} style={{ marginRight: 8 }} /> :
                                    <MaterialCommunityIcons name="email-send-outline" size={16} color={Colors.accent} style={{ marginRight: 8 }} />}
                                <Text style={styles.resendText}>{resendingEmail ? 'Sending...' : 'Resend Verification Email'}</Text>
                            </TouchableOpacity>
                        )}
                    </View>
                    ...
                    <View style={styles.fieldSection}>
                        ...
                        <View style={styles.infoCard}>
                            <View style={styles.infoRow}>
                                <MaterialCommunityIcons name="calendar-outline" size={16} color={Colors.textTertiary} />
                            ...
                            <View style={styles.infoRow}>
                                ...
                                <Text style={[styles.infoValue, { fontSize: 11, fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace' }]} numberOfLines={1}>
...
const styles = {
    header: {
        ...
        paddingTop: Spacing.headerTop + 4, paddingBottom: Spacing.md,
        backgroundColor: Colors.background, borderBottomWidth: 1, borderBottomColor: Colors.borderSubtle,
    },
    ...
    successToast: {
        ...
        backgroundColor: Colors.successSurface, paddingVertical: 10, paddingHorizontal: 16,
        marginHorizontal: Spacing.xl, marginTop: Spacing.sm, borderRadius: Radius.sm,
    },
    avatarSection: { alignItems: 'center', marginBottom: 32, marginTop: 8 },
    avatarWrap: { position: 'relative', marginBottom: 12 },
    avatar: { width: 100, height: 100, borderRadius: 34, backgroundColor: Colors.surfaceElevated },
    avatarPlaceholder: { justifyContent: 'center', alignItems: 'center', backgroundColor: Colors.accent },
    ...
    fieldLabel: { fontSize: 11, fontWeight: '700', color: Colors.textTertiary, letterSpacing: 0.5, marginBottom: 10 },
    ...
    verifiedBadge: {
        flexDirection: 'row', alignItems: 'center', paddingHorizontal: 8, paddingVertical: 4, borderRadius: 6,
    },
    verifiedActive: { backgroundColor: Colors.successSurface },
    verifiedInactive: { backgroundColor: Colors.warningSurface },
    resendBtn: {
        flexDirection: 'row', alignItems: 'center', marginTop: 10,
        backgroundColor: Colors.accentSurface, paddingVertical: 12, paddingHorizontal: 16,
    ...
    infoLabel: { color: Colors.textTertiary, fontSize: 13, marginLeft: 10, flex: 1 },
    ...
    errorBanner: {
        ...
        padding: 12, borderRadius: Radius.sm, marginBottom: 16,
    },
};
