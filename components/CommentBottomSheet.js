...
export default function CommentBottomSheet({
  ...
  const fetchComments = async () => {
    ...
    try {
      const {
        ...
        lastDoc: newLastDoc,
        hasMore: newHasMore,
      } = await IssueService.getComments(issueId, 50, null);
  ...
  const loadMoreComments = async () => {
    ...
    try {
      const {
        ...
        lastDoc: newLastDoc,
        hasMore: newHasMore,
      } = await IssueService.getComments(issueId, 50, lastDoc);
      setComments((prev) => {
  ...
  const handleAddComment = async () => {
    console.log(
      "handleAddComment triggered. commentText:",
      commentText,
      "user:",
      user?.uid,
      "submitting:",
    ...
    if (!commentText.trim() || !user || submitting) {
      ...
      if (!user) Alert.alert("You must be logged in to comment.");
      return;
    }
    ...
    try {
      ...
      Alert.alert("Failed to submit comment. Please try again.");
    } finally {
      setSubmitting(false);
    }
  };
