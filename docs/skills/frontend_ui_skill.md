...
- **Optimistic Updates:** When a user interacts with a high-frequency action (like Upvoting or Joining as a Solver), update the UI state _immediately_ before the Firebase backend call finishes. Roll back the state only if the call fails.
