...
- **Firestore Array Forgery:** When validating arrays (like `voters` or `solvers`) in rules, always check that the user wasn't _already_ in the array before allowing an increment: `!resource.data.arrayName.hasAny([request.auth.uid])`.
...
When proposing a fix, provide the exact file paths and explain _why_ the bug occurred before replacing the code. Do not implement temporary bypasses for security rules unless explicitly requested for local testing, and always revert them.
